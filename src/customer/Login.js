...
import { ScrollView, StatusBar, StyleSheet, TouchableOpacity, TouchableWithoutFeedback, FlatList } from "react-native";
import { Button, Snackbar, TextInput, Colors } from "react-native-paper";
import { NavigationBar, Screen, Title, View, Subtitle, Heading } from "@shoutem/ui";
...
export default class Login extends React.Component {
  ...
  componentDidMount() {
    ...
    const number = state.params.mobile;
    ...
    this.state.add4 === "")
    {
      ...
      then(fcmToken => {
        if (fcmToken) {
          ...
          //"ההרשמה נכשלה"
          this.setState({ alertContent: i18n.t(k._56), showAlert: true, smp: false });
        }
      });
    }
  }
  ...
  renderRowSug(item, index) {
    return (
      <View style={{ flexDirection: 'row', alignContent: 'flex-start', alignSelf: 'flex-start', justifyContent: 'flex-start', marginVertical: sizeHeight(1), marginHorizontal: sizeWidth(6), width: '100%' }}>
  ...
  render() {
    return (
      ...
      style={{
        ...
        leftComponent={<View styleName="horizontal space-between" style={{ marginStart: 12 }}>
            <TouchableOpacity
        ...
        <ScrollView showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
          <View style={{ marginHorizontal: sizeWidth(4) }}>
            <Subtitle style={{ color: '#292929', fontSize: 16, alignSelf: 'flex-start' }}>{i18n.t(k._49)}</Subtitle>
            <TextInput
            dense={true}
            ...
            <Subtitle styleName='v-center h-center' style={{ color: '#292929', fontSize: 16, alignSelf: 'flex-start', marginTop: sizeHeight(1) }}>{i18n.t(k._50)}</Subtitle>
            <TextInput
            style={styles.inputStyle}
            mode={"flat"}
            ...
            <Subtitle styleName='v-center h-center' style={{ color: '#292929', fontSize: 16, alignSelf: 'flex-start', marginTop: sizeHeight(1) }}>{i18n.t(k._51)}</Subtitle>
            <TextInput
            style={styles.inputStyle}
            mode={"flat"}
            ...
            <Subtitle styleName='v-center h-center' style={{ color: '#292929', fontSize: 16, alignSelf: 'flex-start', marginTop: sizeHeight(1) }}>{i18n.t(k._52)}</Subtitle>
            <TextInput
            style={styles.inputStyle}
            mode={"flat"}
            ...
            {/* <Subtitle styleName='v-center h-center' style={{ color: '#292929', fontSize: 16, alignSelf: 'flex-start', marginTop: sizeHeight(1)}}>LandMark</Subtitle>
               <TextInput
                ...
                underlineColorAndroid={"transparent"}
               /> */}
            <Subtitle styleName='v-center h-center' style={{ color: '#292929', fontSize: 16, alignSelf: 'flex-start', marginTop: sizeHeight(1) }}>{i18n.t(k._14)}</Subtitle>
            <TextInput
            style={styles.inputStyle}
            mode={"flat"}
            ...
            {this.state.citiesList.length > 0 ? <View style={{ flexGrow: 1, flexWrap: 'wrap', marginVertical: sizeHeight(2) }}>
              <FlatList
            ...
            <Subtitle styleName='v-center h-center' style={{ color: '#292929', fontSize: 16, alignSelf: 'flex-start', marginTop: sizeHeight(1) }}>{i18n.t(k._53)}</Subtitle>
            <TextInput
        ...
        onPress={() => this.onSaveClick()}>
          ...
          <Subtitle
          style={{ color: "white" }}>{this.state.progressView === true ? i18n.t(k._47) : i18n.t(k._54)}</Subtitle>
        </Button>
        <Snackbar
        visible={
        ...
        {this.state.showAlert ? <AlertDialog isShow={true} title={i18n.t(k._29)} content={this.state.alertContent} callbacks={() => this.setState({ showAlert: false })} /> : null}
