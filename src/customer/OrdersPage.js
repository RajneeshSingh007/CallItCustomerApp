...
"@shoutem/ui";
import { Card, Button, Checkbox, Dialog, Portal, Colors, TextInput, List } from "react-native-paper";
...
export default class OrdersPage extends React.Component {
  constructor(props) {
    ...
    this.serviceName = this.serviceName.bind(this);
    ...
    Pref.getVal(Pref.bearerToken, value => {
      ...
      result => {
        ...
        Object.keys(againgroup).map(keyx => {
          ...
          let finalPricess = Lodash.sumBy(pppp, function (o) {return o.price;});
        ...
        const opFinalOrdersHistory = Lodash.orderBy(allDatasH, ['keys'], ['desc']);

        //ההזמנה מוכנה לאיסוף
        this.setState({ branches: result.branches, progressView: false, eachTabData: boolm ? opFinalOrders : opFinalOrdersHistory, historyData: opFinalOrdersHistory, clonedata: opFinalOrders });
      },
      error => {
  ...
  extraRow(eachTabData, index) {
    return (
      <Row styleName="vertical" styleName="small">
              ...
              fontWeight: '700',
              lineHeight: 20 }}>
              {eachTabData.price}{i18n.t(k._66)}</Title>
					</View>
					<View styleName="horizontal space-between">
            ...
            ...Platform.select({
              ...
              default: {
                ...
                shadowRadius: 1 } }) }}>
                     ...
                     /> */}
						<View style={{ flexDirection: 'column', marginTop: 8, marginStart: 8, marginBottom: 8 }}>
							<Title styleName='bold' style={{
                color: '#292929',
                fontFamily: 'Rubik',
                fontSize: 16,
                ...
                {item.title}</Title>
                  ...
                  return index < 3 ? <Subtitle styleName='bold' style={{
                    color: '#292929',
                    fontFamily: 'Rubik',
                    fontSize: 15,
                    alignSelf: 'flex-start',
                    fontWeight: '400' }}>
                    {`${Lodash.capitalize(ele)}`}</Subtitle> : null;
                }) : null}
							</View>
                ...
                fontSize: 14,
                fontWeight: '400' }}>
                {`${i18n.t(k._74)} `}<Subtitle style={{
                  color: '#292929',
                  fontFamily: 'Rubik',
                  alignSelf: 'flex-start',
                  fontSize: 14,
                  fontWeight: '700' }}>
                  {`${i18n.t(k._6)}${item.totalPrice}`}</Subtitle></Subtitle>
							<Subtitle style={{
                color: '#6DC124',
                fontFamily: 'Rubik',
                alignSelf: 'flex-start',
                fontSize: 14 }}>
                {item.paid !== 0 ? `${i18n.t(k._75)}` : `${i18n.t(k._76)}`}</Subtitle>
							<View style={{
                height: 1,
                marginEnd: 6,
                ...
                fontSize: 12,
                paddingVertical: 2 }}>
                {item.status === -1 ? `${i18n.t(k.cancelText)}` : item.status === 1 ? `${i18n.t(k._77)}` : item.status === 2 ? i18n.t(k._78) : item.status === 3 ? item.isdelivery === 0 ? `${i18n.t(k.difff)}` : `${i18n.t(k.deliveryText)}` : `${i18n.t(k.orderdoneText)}`}</Subtitle>
      ...
      style={{
        ...
        style={
        {
          ...
          height: sizeHeight(9) }}>
					...
					<TouchableWithoutFeedback onPress={() => this.setState({ selectedMode: true, eachTabData: this.state.clonedata })}>
                ...
                {`${i18n.t(k._80)}`}</Title>
					...
					<TouchableWithoutFeedback onPress={() => this.setState({ selectedMode: false, eachTabData: this.state.historyData })}>
						<View style={{ flex: 0.5, backgroundColor: !this.state.selectedMode ? '#5EBBD7' : 'white', alignContent: 'center', justifyContent: 'center', alignItems: 'center' }}>
							<Title styleName='bold' style={{
                color: !this.state.selectedMode ? 'white' : '#777777',
                fontFamily: 'Rubik',
        ...
        style={{
          ...
          center={
          this.state.eachTabData !== null && this.state.eachTabData.length > 0 ?
          <FlatList
          extraData={this.state}
          showsHorizontalScrollIndicator={false}
          showsVerticalScrollIndicator={false}
          ...
          message={this.state.selectedMode ? i18n.t(k._82) : i18n.t(k._83)} />} />
