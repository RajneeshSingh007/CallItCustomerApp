...
"@shoutem/ui";
import { Card, Button, Checkbox, Dialog, Portal, Colors, TextInput } from "react-native-paper";
...
export default class ReviewsPage extends React.Component {
  ...
  fetchReviews() {
    ...
    Pref.getVal(Pref.bearerToken, value => {
      ...
      result => {
        console.log(result);
        this.setState({ branches: result.branches, progressView: false, eachTabData: result });
      },
      error => {
        ...
        marginVertical: sizeHeight(1) }}>
          ...
          fullStarColor={'#EFCE4A'} />
            ...
            style={{
              ...
              marginStart: 4,
              fontWeight: '400' }}>
              {this.bindDate(item.date) + "   " + item.username}</Subtitle>
                    </View>
                </Card>
            </View>);
        ...
        backgroundColor: "white" }}>
          ...
          /> : null}
            ...
            null}
                        ...
                        <View style={{
              flexDirection: 'column', paddingHorizontal: sizeWidth(1),
              paddingVertical: sizeHeight(2) }}>
                            <Subtitle style={{ color: '#777777', fontSize: 16, alignSelf: 'flex-start', marginStart: sizeWidth(4) }}>{'כמה כוכבים?'}</Subtitle>
                            <StarRating
              disabled={false}
              emptyStar={'ios-star-outline'}
              fullStar={'ios-star'}
              ...
              containerStyle={{ justifyContent: i18n.t(k.FLEX_START), marginHorizontal: sizeWidth(4), marginVertical: sizeHeight(0.2) }}
              rating={this.state.rating}
              selectedStar={this.onStarRatingPress}
              fullStarColor={'#EFCE4A'}
              //reversed={true}
              />
                            <Subtitle style={{ color: '#777777', fontSize: 16, alignSelf: 'flex-start', marginStart: sizeWidth(4) }}>{'מה אתה חושב על העסק?'}</Subtitle>
                            <TextInput
              mode='flat'
            ...
            style={{
              alignSelf: "center" }}>
                ...
                {this.state.showAlert ? <AlertDialog isShow={true} title={i18n.t(k._30)} content={this.state.alertContent} callbacks={() => this.setState({ showAlert: false })} /> : null}
